...
#file_path = 'F:\\python_test\\test\\sata_link.vhd'
...
vhdl_module_pattern = r'^\s*(?:\bentity\b)\s*(\w+)(?:\s*is\s*)'
vhdl_start_pattern  = r'^\s*library|LIBRARY\s*\w+\s*[;]\s*'
vhdl_port_pattern   = r'^\s*(\w+)\s*(?:[:])\s*(in\b|out|inout)\s*(\w+)*\s*([(]\s*\S+\s*\s*\-*\s*\w*\s*downto\s*\S+\s*\s*\-*\s*\w*\s*[)])*\s*[;]*\s*'
vhdl_param_pattern  = r'^\s*(\w+)\s*[:]\s*(\w+)\s*([(]\s*\S+\s*\s*\-*\s*\w*\s*downto\s*\S+\s*\s*\-*\s*\w*\s*[)])*\s*[:]\s*[=]\s*((\w+)(["]\w+["])*)\s*'
